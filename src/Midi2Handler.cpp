...
bool Midi2Handler::processIncoming(const juce::MidiMessage& msg)
{
    ...
    // Destination MUID (bytes 9-12) — verify message is addressed to us or broadcast
    ...
    bool isForUs = (memcmp(dstMuid, ourMuid, 4) == 0);
    ...
    switch (subId2)
    {
        ...
        case 0x36: // Set Property Data (Keystage writing parameter values to us)
        {
            ...
            // Find the body
            int bodyOffset = 16 + headerLen + 4; // skip header + 2-byte chunk count + 2-byte chunk num
            if (bodyOffset + 2 <= size)
            {
                int bodyLen = data[bodyOffset] | (data[bodyOffset + 1] << 7);
                bodyOffset += 2;
                juce::String bodyStr;
                for (int i = 0; i < bodyLen && (bodyOffset + i) < size; ++i)
                    bodyStr += juce::String::charToString(static_cast<char>(data[bodyOffset + i]));

                // Apply values if this is X-ProgramEdit and we have a plugin
                if (headerStr.toLowerCase().contains("x-programedit") && currentPlugin != nullptr)
                {
                    auto json = juce::JSON::parse(bodyStr);
                    if (auto* obj = json.getDynamicObject())
                    {
                        if (auto* arr = obj->getProperty("currentValues").getArray())
                        {
                            auto& params = currentPlugin->getParameters();
                            for (auto& entry : *arr)
                            {
                                if (auto* item = entry.getDynamicObject())
                                {
                                    juce::String name = item->getProperty("name").toString();
                                    int val = static_cast<int>(item->getProperty("value"));
                                    for (auto& m : mappings)
                                    {
                                        if (m.name == name && m.pluginParamIndex >= 0
                                            && m.pluginParamIndex < params.size())
                                        {
                                            params[m.pluginParamIndex]->setValue(
                                                static_cast<float>(val) / 127.0f);
                                            break;
                                        }
                                    }
                                }
            ...
            // Reply: Set Property Data Ack (0x37)
            {
                ...
                addCISysEx(0x37, srcMuid, payload);
            }
            return true;
        }
        ...
        case 0x38: // Subscription request
        {
            ...
            if (subLower.contains("x-programedit") && subLower.contains("start"))
            {
                ...
                // Store subscribeId from header (Keystage provides it)
                {
                    auto hdrJson = juce::JSON::parse(subHeader);
                    if (auto* obj = hdrJson.getDynamicObject())
                    {
                        juce::String sid = obj->getProperty("subscribeId").toString();
                        if (sid.isNotEmpty())
                            subscribeId = sid;
                    }
                }

                juce::String replyHeader = "{\"status\":200,\"subscribeId\":\"" + subscribeId
                                         + "\",\"command\":\"start\",\"resource\":\"X-ProgramEdit\"}";
        ...
        {
            // Keystage is resetting — clear our connection state and re-discover
            memset(keystageMuid, 0, 4);
            hasXProgramEditSubscription = false;
            subscribeId = "xpe1";
            sendDiscovery(); // auto-reconnect
            return true;
        }
...
void Midi2Handler::pushParameterListToKeystage()
{
    ...
    // Use Get Property Data Reply (0x35) — proactive push of our X-ParameterList resource
    juce::String header = "{\"status\":200,\"resource\":\"X-ParameterList\",\"mediaType\":\"application/json\"}";
    juce::String body = buildParameterList();
    sendPropertyReply(keystageMuid, 0x01, header, body);
}
...
void Midi2Handler::pushProgramEditToKeystage()
{
    ...
    if (hasXProgramEditSubscription)
    {
        // Notify via subscription (Keystage is watching for updates)
        sendParameterUpdate();
    }
    else
    {
        // Use Get Property Data Reply (0x35) — proactive push before subscription is established
        juce::String header = "{\"status\":200,\"resource\":\"X-ProgramEdit\",\"mediaType\":\"application/json\"}";
        juce::String body = buildProgramEdit();
        sendPropertyReply(keystageMuid, 0x02, header, body);
    }
}
...
void Midi2Handler::sendParameterUpdate()
{
    ...
    juce::String header = "{\"status\":200,\"subscribeId\":\"" + subscribeId + "\",\"command\":\"notify\"}";
