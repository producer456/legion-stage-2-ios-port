...
public:
    ...
    struct Callbacks
    {
        ...
        // Direct plugin param write — absolute param index, bypasses MIDI mapping
        std::function<void(int absParamIndex, float value)> onDirectParamChange;
    ...
    // Solo mode: all plugin params for paged browsing (shift key cycles pages)
    void setAllParams(const juce::StringArray& names, const juce::Array<float>& values);
    ...
    // Called from the 15Hz UI timer — drives beat flash + REC blink
    void animationTick(double positionInBeats, bool isPlaying, bool isRecording);
...
private:
    ...
    void setWheelColor(uint8_t r, uint8_t g, uint8_t b);   // sets base color + sends
    void sendWheelColorRaw(uint8_t r, uint8_t g, uint8_t b); // sends without updating base
    ...
    static juce::String makeBar4(float val); // 4-char block bar e.g. "██░░"
    ...
    // All-param paged browsing (Solo mode)
    juce::StringArray      allParamNames;
    juce::Array<float>     allParamValues;
    ...
    // Beat flash + REC blink animation state
    double      lastTickBeatPos      = -1.0;
    juce::int64 beatFlashEndMs       = 0;
    juce::int64 lastStoppedPulseMs   = 0;
    juce::int64 modeAnnouncedUntilMs = 0;
    bool        wasFlashingWheel   = false;
    bool        wasRecording       = false;
    bool        lastRecBlinkState  = false;
    uint8_t     baseWheelR = 30, baseWheelG = 255, baseWheelB = 30; // default: Companion green
    static constexpr juce::int64 BEAT_FLASH_MS = 150;
