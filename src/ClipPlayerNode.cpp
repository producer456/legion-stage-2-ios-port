...
void ClipPlayerNode::processBlock(juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midi)
{
    ...
    if (engine.isPlaying() && engine.isRecording() && !engine.isInCountIn() && armed.load() && recordingSlot < 0)
    {
        ...
        if (targetSlot >= 0)
        {
            ...
            // Fix 3: pre-size clip to loop length so it doesn't grow beat-by-beat
            if (engine.isLoopEnabled() && engine.hasLoopRegion())
            {
                double loopLength = engine.getLoopEnd() - engine.getLoopStart();
                slot.clip->lengthInBeats = std::ceil(loopLength / 4.0) * 4.0;
                if (slot.clip->lengthInBeats < 4.0) slot.clip->lengthInBeats = 4.0;
            }
        }
    }

    // Fix 2: if REC was toggled off while overdubbing, finalize the recording now
    if (recordingSlot >= 0 && !engine.isRecording())
        finalizeRecording();

    // Detect position jump (loop wrap or seek) before recording/playback
    double currentPos = engine.getPositionInBeats();
    bool positionJumped = engine.isPlaying() && !engine.isInCountIn()
                          && (currentPos < lastPositionInBeats - 0.001);

    // On loop wrap while recording: extend clip back to loopStart, shift existing
    // events forward, then switch to Playing so the clip plays back while overdubbing.
    if (positionJumped && recordingSlot >= 0 && engine.isLoopEnabled() && engine.hasLoopRegion())
    {
        auto& slot = slots[static_cast<size_t>(recordingSlot)];
        if (slot.clip != nullptr)
        {
            double loopStart = engine.getLoopStart();
            double extensionBeats = recordStartBeat - loopStart;
            if (extensionBeats > 0.0)
            {
                for (int e = 0; e < slot.clip->events.getNumEvents(); ++e)
                {
                    auto* holder = slot.clip->events.getEventPointer(e);
                    holder->message.setTimeStamp(holder->message.getTimeStamp() + extensionBeats);
                }
                slot.clip->timelinePosition = loopStart;
                double loopLength = engine.getLoopEnd() - loopStart;
                if (slot.clip->lengthInBeats < loopLength)
                    slot.clip->lengthInBeats = std::ceil(loopLength / 4.0) * 4.0;
                recordStartBeat = loopStart;
            }
    ...
    {
        // Kill sounding notes on position jump (loop wrap or seek)
        if (positionJumped)
        {
...
void ClipPlayerNode::finalizeRecording()
{
    ...
    // If still in Recording state (never wrapped to overdub), transition to Playing
    if (slot.state.load() == ClipSlot::Recording)
        slot.state.store(slot.hasContent() ? ClipSlot::Playing : ClipSlot::Empty);
    // If already Playing (overdub after loop wrap), stay Playing
}
