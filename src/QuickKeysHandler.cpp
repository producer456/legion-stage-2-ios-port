...
bool QuickKeysHandler::openDevice()
{
    ...
    for (DWORD i = 0; SetupDiEnumDeviceInterfaces(devInfo, nullptr, &hidGuid, i, &interfaceData); ++i)
    {
        ...
        {
            // Log any Xencelabs VID device found so unexpected PIDs (e.g. new BT firmware) show up in the log.
            if (attrs.VendorID == VENDOR_ID)
                logFile.appendText("Found Xencelabs device VID=0x28BD PID=0x"
                    + juce::String::toHexString(attrs.ProductID) + "\n");
    ...
    while (!threadShouldExit())
    {
        ...
        if (!result && err == ERROR_IO_PENDING)
        {
            ...
            if (waitResult == WAIT_TIMEOUT)
            {
                ...
                // Skip if a mode splash is currently showing
...
void QuickKeysHandler::sendWheelColorRaw(uint8_t r, uint8_t g, uint8_t b)
{
    uint8_t buf[HID_BUFFER_SIZE] = {};
    buf[0] = REPORT_ID;
    buf[1] = CMD_WHEEL;
...
void QuickKeysHandler::setWheelColor(uint8_t r, uint8_t g, uint8_t b)
{
    ...
    baseWheelG = g;
    baseWheelB = b;
    sendWheelColorRaw(r, g, b);
}

juce::String QuickKeysHandler::makeBar4(float val)
{
    int filled = juce::jlimit(0, 4, juce::roundToInt(val * 4.0f));
    juce::String s;
    for (int i = 0; i < 4; ++i)
        s += (i < filled) ? juce::juce_wchar(0x2588) : juce::juce_wchar(0x2591);
    return s;
}

void QuickKeysHandler::animationTick(double positionInBeats, bool isPlaying, bool isRecording)
{
    if (!deviceConnected.load()) return;

    juce::int64 now = juce::Time::currentTimeMillis();

    // ── Mode announcement: restore display once the splash expires ────────────
    if (modeAnnouncedUntilMs > 0 && now >= modeAnnouncedUntilMs)
    {
        modeAnnouncedUntilMs = 0;
        refreshDisplay();
    }
    if (modeAnnouncedUntilMs > 0) return;  // still showing splash, skip everything else

    // ── Keep base wheel color in sync with transport state ────────────────────
    if (isRecording != wasRecording || (isPlaying != (lastTickBeatPos >= 0.0)))
        setTransportState(isPlaying, isRecording);

    // ── Beat flash ────────────────────────────────────────────────────────────
    if (isPlaying && lastTickBeatPos >= 0.0)
    {
        // Beat-synced flash while playing
        if (static_cast<int>(positionInBeats) > static_cast<int>(lastTickBeatPos))
            beatFlashEndMs = now + BEAT_FLASH_MS;
    }
    else if (!isPlaying && isRecording)
    {
        // Stopped but record armed — pulse at fixed rate so user knows it's still on
        static constexpr juce::int64 STOPPED_REC_PULSE_MS = 650;
        if (now - lastStoppedPulseMs >= STOPPED_REC_PULSE_MS)
        {
            lastStoppedPulseMs = now;
            beatFlashEndMs = now + BEAT_FLASH_MS;
        }
    }
    lastTickBeatPos = isPlaying ? positionInBeats : -1.0;

    bool flashing = (now < beatFlashEndMs);
    if (flashing != wasFlashingWheel)
    {
        wasFlashingWheel = flashing;
        if (flashing)
            sendWheelColorRaw(isRecording ? 255 : 255,   // recording = bright red flash
                              isRecording ?  60 : 255,   // playing   = white flash
                              isRecording ?  60 : 255);
        else
            sendWheelColorRaw(baseWheelR, baseWheelG, baseWheelB);
    }

    // ── REC blink (Companion mode only) ──────────────────────────────────────
    if (currentMode.load() == Mode::Companion)
    {
        if (isRecording)
        {
            bool blink = (now / 400) % 2 == 0;
            if (blink != lastRecBlinkState || !wasRecording)
            {
                lastRecBlinkState = blink;
                setKeyText(2, blink ? " REC" : "*REC");
            }
        }
        else if (wasRecording)
        {
            // Recording just stopped — restore label
            setKeyText(2, "REC");
        }
    }
...
void QuickKeysHandler::processInputReport(const uint8_t* data, int size)
{
    ...
    if (subType == INPUT_REPORT_KEYS)
    {
        ...
        uint8_t wheel = (size > 7) ? data[7] : 0;

        // Only update key states on pure key reports (wheel == 0).
        // Wheel reports may not carry the full key bitmask, which would
        // cause false button-release events (e.g. shift dropping mid-scroll).
        if (wheel == 0)
        {
...
void QuickKeysHandler::handleButtonRelease(int buttonIndex)
{
    if (buttonIndex == 8)
    {
        ...
        if (!shiftUsed)
        {
            ...
            {
                // Long press = enter/exit Visuals mode
                if (currentMode.load() == Mode::Visuals)
                    setMode(Mode::Solo);
                else
                    setMode(Mode::Visuals);
            }
            else if (currentMode.load() == Mode::Solo && !allParamNames.isEmpty())
            {
                // Short tap in Solo = cycle param page
                cycleParamPage();
            }
        }
        shiftUsed = false;
...
void QuickKeysHandler::handleSoloWheel(int direction)
{
    if (shiftHeld)
    {
        if (currentMode.load() == Mode::Solo && !allParamNames.isEmpty())
        {
            shiftUsed = true;  // prevent visuals entry on release
            // Shift + wheel = param page navigation in Solo mode
            if (direction > 0)
            {
                int total    = allParamNames.size();
                int numPages = (total + NUM_LABELED_BUTTONS - 1) / NUM_LABELED_BUTTONS;
                int nextPage = (paramPageOffset / NUM_LABELED_BUTTONS + 1) % numPages;
                paramPageOffset = nextPage * NUM_LABELED_BUTTONS;
            }
            else
            {
                int total    = allParamNames.size();
                int numPages = (total + NUM_LABELED_BUTTONS - 1) / NUM_LABELED_BUTTONS;
                int prevPage = (paramPageOffset / NUM_LABELED_BUTTONS - 1 + numPages) % numPages;
                paramPageOffset = prevPage * NUM_LABELED_BUTTONS;
            }
            loadCurrentPage();
            int numPages = (allParamNames.size() + NUM_LABELED_BUTTONS - 1) / NUM_LABELED_BUTTONS;
            int curPage  = paramPageOffset / NUM_LABELED_BUTTONS;
            showOverlayText("Page " + juce::String(curPage + 1) + "/" + juce::String(numPages), 2);
        }
        else
        {
            // Shift + wheel = MIDI page navigation in other modes
    ...
    if (selectedParamIndex >= 0 && selectedParamIndex < 8)
    {
        ...
        // Also keep allParamValues in sync
        int absIdx = paramPageOffset + selectedParamIndex;
        if (absIdx < allParamValues.size())
            allParamValues.set(absIdx, val);

        // Prefer direct param write if we have all params loaded
        if (callbacks.onDirectParamChange && absIdx < allParamValues.size())
        {
            float capturedVal = val;
            juce::MessageManager::callAsync([this, absIdx, capturedVal] {
                if (callbacks.onDirectParamChange)
                    callbacks.onDirectParamChange(absIdx, capturedVal);
            });
        }
        ...
        // Update label and wheel color — rate-limited
        juce::int64 now = juce::Time::currentTimeMillis();
        if (now - lastWheelColorSentMs >= WHEEL_COLOR_RATE_MS)
        {
            lastWheelColorSentMs = now;
            refreshSoloDisplay();  // updates name+% in place, no flash
...
void QuickKeysHandler::setMode(Mode m)
{
    ...
    switch (m)
    {
        case Mode::Solo:      modeName = "Solo";       break;
        case Mode::Companion: modeName = "Transport";  break;
        case Mode::Edit:      modeName = "Edit";       break;
        case Mode::Extras:    modeName = "Extras";     break;
        case Mode::Visuals:   modeName = "Visuals";    break;
        default:              modeName = "Unknown";    break;
    }

    modeAnnouncedUntilMs = juce::Time::currentTimeMillis() + 1000;
    refreshDisplay();
    showOverlayText(modeName, 1);
...
void QuickKeysHandler::setAllParams(const juce::StringArray& names, const juce::Array<float>& values)
{
    allParamNames  = names;
    allParamValues = values;
    paramPageOffset = 0;
    loadCurrentPage();
}

void QuickKeysHandler::loadCurrentPage()
{
    int total = allParamNames.size();
    for (int i = 0; i < NUM_LABELED_BUTTONS; ++i)
    {
        int absIdx = paramPageOffset + i;
        currentParamNames.set(i, absIdx < total ? allParamNames[absIdx] : juce::String());
        paramValues[i]    = absIdx < total ? allParamValues[absIdx] : 0.5f;
    }
    selectedParamIndex = 0;
    refreshSoloDisplay();
}

void QuickKeysHandler::cycleParamPage()
{
    int total = allParamNames.size();
    if (total == 0) return;

    int numPages = (total + NUM_LABELED_BUTTONS - 1) / NUM_LABELED_BUTTONS;
    int currentPage = paramPageOffset / NUM_LABELED_BUTTONS;
    int nextPage    = (currentPage + 1) % numPages;
    paramPageOffset = nextPage * NUM_LABELED_BUTTONS;

    loadCurrentPage();

    juce::String msg = "Page " + juce::String(nextPage + 1) + "/" + juce::String(numPages);
    showOverlayText(msg, 2);
}
