#include "MainComponent.h"
 ...
 // Minimal COM smart pointer (avoids ATL dependency)
 template<class T>
 struct ComPtr
 {
     T* p = nullptr;
...
void MainComponent::timerCallback()
{
    ...
    // Repaint OLED-animated buttons whenever they are active (on = true)
    if (eng.isPlaying())
    {
        playButton.repaint();
        stopButton.repaint();
    }
    auto repaintIfOn = [](juce::TextButton& btn) { if (btn.getToggleState()) btn.repaint(); };
    repaintIfOn(metronomeButton);
    repaintIfOn(loopButton);
    repaintIfOn(countInButton);
    repaintIfOn(midiLearnButton);
    repaintIfOn(mixerButton);
    repaintIfOn(pianoToggleButton);
    repaintIfOn(fullscreenButton);   // "VIS"
    repaintIfOn(projectorButton);    // "PROJ"
    {
    ...
    // isOverdubbing() covers the post-wrap Playing+recording state so the
    // snapshot doesn't fire prematurely on loop wrap.
    ...
    for (int t = 0; t < PluginHost::NUM_TRACKS; ++t)
    {
        ...
        if (cp != nullptr)
        {
            if (cp->isOverdubbing()) isRec = true;
    ...
    // Drive Quick Keys animations (beat flash, REC blink)
    // isRecActive = armed OR actively recording, so wheel flashes red whenever record is on
    if (quickKeysEnabled && quickKeysHandler.isDeviceConnected())
        quickKeysHandler.animationTick(eng.getPositionInBeats(), eng.isPlaying(),
                                       isRec || recordButton.getToggleState());
...
{
    auto* dialog = new juce::AlertWindow("Save", "Choose format to save:", juce::MessageBoxIconType::NoIcon);
    dialog->addButton("Song File",  1);
    dialog->addButton("WAV",        2);
    dialog->addButton("MP3",        3);
    ...
    dialog->enterModalState(true, juce::ModalCallbackFunction::create([this](int result)
    {
        ...
        {
            bool mp3 = (result == 3);
            juce::String pattern = mp3 ? "*.mp3" : "*.wav";
            juce::String title   = mp3 ? "Export MP3" : "Export WAV";
            ...
            chooser->launchAsync(juce::FileBrowserComponent::saveMode | juce::FileBrowserComponent::canSelectFiles,
                [this, chooser, mp3](const juce::FileChooser& fc)
            {
                auto file = fc.getResult();
                if (file == juce::File()) return;
                exportMixdown(file, mp3);
            });
        }
    }), true);
}

void MainComponent::exportMixdown(juce::File targetFile, bool mp3)
{
    ...
    for (int t = 0; t < PluginHost::NUM_TRACKS; ++t)
    {
        ...
        for (int s = 0; s < ClipPlayerNode::NUM_SLOTS; ++s)
        {
            auto& slot = cp->getSlot(s);
            if (slot.clip != nullptr && slot.hasContent())
                endBeat = juce::jmax(endBeat, slot.clip->timelinePosition + slot.clip->lengthInBeats);
        }
    }
    ...
    if (endBeat <= 0.0)
    {
        juce::AlertWindow::showMessageBoxAsync(juce::MessageBoxIconType::InfoIcon,
            "Export", "Nothing to export — add some clips first.");
        return;
    }

    endBeat += 4.0; // extra bar for reverb/delay tails

    double sampleRate     = pluginHost.getSampleRate();
    int    blockSize      = pluginHost.getBlockSize();
    double bpm            = pluginHost.getEngine().getBpm();
    double beatsPerSample = (bpm / 60.0) / sampleRate;
    juce::int64 totalSamples = static_cast<juce::int64>(endBeat / beatsPerSample) + blockSize;

    // Always render to a temp WAV first, then transcode to MP3 if needed
    juce::File tempWav = juce::File::createTempFile("legstage_export_wav");

    {
        juce::WavAudioFormat wavFmt;
        auto stream = std::make_unique<juce::FileOutputStream>(tempWav);
        if (!stream->openedOk())
        {
            juce::AlertWindow::showMessageBoxAsync(juce::MessageBoxIconType::WarningIcon,
                "Export", "Could not create temp file for rendering.");
            return;
        }

        std::unique_ptr<juce::AudioFormatWriter> writer(
            wavFmt.createWriterFor(stream.get(), sampleRate, 2, 24, {}, 0));

        if (writer == nullptr)
        {
            juce::AlertWindow::showMessageBoxAsync(juce::MessageBoxIconType::WarningIcon,
                "Export", "Could not create WAV writer.");
            return;
        }
        stream.release(); // writer owns it

        // Save engine + slot state
        bool wasPlaying = pluginHost.getEngine().isPlaying();
        double savedPos = pluginHost.getEngine().getPositionInBeats();
        bool loopWasOn  = pluginHost.getEngine().isLoopEnabled();

        struct SlotState { int track, slot; ClipSlot::State state; };
        juce::Array<SlotState> savedSlotStates;
        for (int t = 0; t < PluginHost::NUM_TRACKS; ++t)
        {
            auto* cp = pluginHost.getTrack(t).clipPlayer;
            if (cp == nullptr) continue;
            for (int s = 0; s < ClipPlayerNode::NUM_SLOTS; ++s)
            {
                auto& sl = cp->getSlot(s);
                auto st = sl.state.load();
                savedSlotStates.add({ t, s, st });
                if (sl.hasContent()) sl.state.store(ClipSlot::Playing);
            }
        }

        pluginHost.getEngine().setPosition(0.0);
        if (loopWasOn) pluginHost.getEngine().toggleLoop();
        pluginHost.getEngine().play();
        ...
        {
            int thisBlock = static_cast<int>(juce::jmin((juce::int64)blockSize, totalSamples - samplesWritten));
            buffer.setSize(2, thisBlock, false, true, true);
            buffer.clear();
            midiBuffer.clear();

            pluginHost.processBlock(buffer, midiBuffer);
            writer->writeFromAudioSampleBuffer(buffer, 0, thisBlock);
            samplesWritten += thisBlock;
        }

        // Restore engine + slots
        pluginHost.getEngine().stop();
        if (loopWasOn) pluginHost.getEngine().toggleLoop();
        pluginHost.getEngine().setPosition(savedPos);
        if (wasPlaying) pluginHost.getEngine().play();
        for (auto& ss : savedSlotStates)
        {
            auto* cp = pluginHost.getTrack(ss.track).clipPlayer;
            if (cp != nullptr) cp->getSlot(ss.slot).state.store(ss.state);
        }

        audioPlayer.setProcessor(&pluginHost);
        writer.reset(); // flush + close temp WAV
    }

    if (mp3)
    {
#if JUCE_WINDOWS
        // Transcode temp WAV -> MP3 using Windows Media Foundation
        statusLabel.setText("Encoding MP3...", juce::dontSendNotification);
        ...
        bool ok = [&]() -> bool
        {
            ...
            // Open temp WAV as source
            IMFSourceReader* pReader = nullptr;
            if (FAILED(MFCreateSourceReaderFromURL(tempWav.getFullPathName().toWideCharPointer(),
                                                   nullptr, &pReader))) return false;
            ComPtr<IMFSourceReader> reader(pReader);

            reader->SetStreamSelection(MF_SOURCE_READER_ALL_STREAMS, FALSE);
            reader->SetStreamSelection(MF_SOURCE_READER_FIRST_AUDIO_STREAM, TRUE);

            // Read native type to get sample rate + channels
            IMFMediaType* pNative = nullptr;
            if (FAILED(reader->GetNativeMediaType(MF_SOURCE_READER_FIRST_AUDIO_STREAM, 0, &pNative)))
                return false;
            ComPtr<IMFMediaType> nativeType(pNative);
            UINT32 sr = 44100, ch = 2;
            nativeType->GetUINT32(MF_MT_AUDIO_SAMPLES_PER_SECOND, &sr);
            nativeType->GetUINT32(MF_MT_AUDIO_NUM_CHANNELS, &ch);

            // Build a complete PCM 16-bit input type (all required attributes)
            IMFMediaType* pPcm = nullptr;
            MFCreateMediaType(&pPcm);
            ComPtr<IMFMediaType> pcmType(pPcm);
            ...
            pcmType->SetUINT32(MF_MT_AUDIO_NUM_CHANNELS,         ch);
            pcmType->SetUINT32(MF_MT_AUDIO_SAMPLES_PER_SECOND,   sr);
            pcmType->SetUINT32(MF_MT_AUDIO_BLOCK_ALIGNMENT,      ch * 2);
            pcmType->SetUINT32(MF_MT_AUDIO_AVG_BYTES_PER_SECOND, sr * ch * 2);
            pcmType->SetUINT32(MF_MT_ALL_SAMPLES_INDEPENDENT,    TRUE);

            if (FAILED(reader->SetCurrentMediaType(MF_SOURCE_READER_FIRST_AUDIO_STREAM,
                                                   nullptr, pcmType))) return false;

            // Build MP3 output type
            IMFMediaType* pMp3 = nullptr;
            MFCreateMediaType(&pMp3);
            ComPtr<IMFMediaType> mp3Type(pMp3);
            mp3Type->SetGUID(MF_MT_MAJOR_TYPE,                   MFMediaType_Audio);
            mp3Type->SetGUID(MF_MT_SUBTYPE,                      MFAudioFormat_MP3);
            mp3Type->SetUINT32(MF_MT_AUDIO_NUM_CHANNELS,         ch);
            mp3Type->SetUINT32(MF_MT_AUDIO_SAMPLES_PER_SECOND,   sr);
            mp3Type->SetUINT32(MF_MT_AUDIO_AVG_BYTES_PER_SECOND, 320000 / 8); // 320 kbps
            mp3Type->SetUINT32(MF_MT_AUDIO_BLOCK_ALIGNMENT,      1);

            // Create sink writer for MP3 file
            IMFSinkWriter* pWriter = nullptr;
            if (FAILED(MFCreateSinkWriterFromURL(targetFile.getFullPathName().toWideCharPointer(),
                                                 nullptr, nullptr, &pWriter))) return false;
            ComPtr<IMFSinkWriter> sinkWriter(pWriter);

            DWORD streamIdx = 0;
            if (FAILED(sinkWriter->AddStream(mp3Type, &streamIdx)))        return false;
            if (FAILED(sinkWriter->SetInputMediaType(streamIdx, pcmType, nullptr))) return false;
            if (FAILED(sinkWriter->BeginWriting()))                         return false;
            ...
            for (;;)
            {
                ...
                if (pSample)
                {
                    ComPtr<IMFSample> sample(pSample);
                    sample->SetSampleTime(timestamp);
                    sinkWriter->WriteSample(streamIdx, sample);
                }
            }

            return SUCCEEDED(sinkWriter->Finalize());
        }();
        ...
        {
            // Keep the render as WAV so the user doesn't lose their work
            juce::File wavFallback = targetFile.withFileExtension("wav");
            tempWav.moveFileTo(wavFallback);
            juce::AlertWindow::showMessageBoxAsync(juce::MessageBoxIconType::WarningIcon,
                "Export", "MP3 encoding failed — saved as WAV instead:\n" + wavFallback.getFileName());
            return;
        }
#endif
    }
    ...
    {
        // WAV — just move the temp file to the destination
        targetFile.deleteFile();
        tempWav.moveFileTo(targetFile);
    }

    statusLabel.setText("Exported: " + targetFile.getFileName(), juce::dontSendNotification);
}
...
void MainComponent::syncQuickKeysParams()
{
    ...
    // Feed automatable plugin params for paged browsing via shift key
    // Skip non-automatable and meta params (MIDI routing, internal state, etc.)
    if (track.plugin)
    {
        auto& params = track.plugin->getParameters();
        juce::StringArray allNames;
        juce::Array<float> allValues;
        for (auto* p : params)
        {
            if (!p->isAutomatable() || p->isMetaParameter())
                continue;
            allNames.add(p->getName(32));
            allValues.add(p->getValue());
        }
        quickKeysHandler.setAllParams(allNames, allValues);
    }

    // Also keep MIDI-mapping names/values in sync for the legacy onParamChange path
    auto& mappings = midi2Handler.getMappings();
...
void MainComponent::setupQuickKeysCallbacks()
{
    ...
    cb.onDirectParamChange = [this](int absParamIndex, float value) {
        auto& trk = pluginHost.getTrack(selectedTrackIndex);
        if (trk.plugin)
        {
            auto& params = trk.plugin->getParameters();
            if (absParamIndex >= 0 && absParamIndex < params.size())
            {
                params[absParamIndex]->setValue(value);
                updateParamSliders();
            }
        }
    };
